...
def scroll_plan_z(cam_pos, vertices, tile_number, tile_length):  # not working
    tile_number += 1
    if cam_pos < vertices[-1][2]:
        for i in range(tile_number):
            j = i * tile_number + tile_number - 1
            k = i * tile_number
            del vertices[j]
            vertices.insert(k, [vertices[k][0],
                                vertices[k][1],
                                vertices[k][2] - tile_length])
...
def apply_2d_fct(center, tile_number, vertices):
    for i in range(tile_number + 1):
        for j in range(tile_number + 1):
            vertices[j + i * (tile_number + 1)][1] = function(vertices[j + i * (tile_number + 1)][0],
                                                              vertices[j + i * (tile_number + 1)][2]) + center[1]
    return vertices


def generation(h_max, h_min, delta_max, delta_min):
    return
...
def main():
    ...
    # vertices = apply_2d_fct(center, tile_number, vertices)
    ...
    while True:
        ...
        scroll_plan_z(cam_pos_z, vertices, tile_number, tile_length)
